...
public class Add_Attachment {
    ...
    public void addAttachments() throws IOException {
        File f=new File("src/test/java/document/Screenshot 2023-12-26 200114.png");
        Response response=RestAssured.given().baseUri(ReadProperties.readProperty("URL")).contentType(ContentType.MULTIPART)
                .multiPart("file",f).header("Cookie",cookievalue).header("X-Atlassian-Token","no-check")
